	...
	)
{
	...
	t->txSize = 0;
	t->tySize = 0;
	t->TVecs.uOffset = 0.0f;
	t->TVecs.uVec.X = 0.0f;
	t->TVecs.uVec.Y = 0.0f;
	t->TVecs.uVec.Z = 0.0f;
	t->TVecs.vOffset = 0.0f;
	t->TVecs.vVec.X = 0.0f;
	t->TVecs.vVec.Y = 0.0f;
	t->TVecs.vVec.Z = 0.0f;
...
//		ConPrintf("Face with no normal!\n");	//	old gedit
		CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("Face with no normal!\n");	//	new g3dc
		return	GE_FALSE;
	}
...
{
	//	this function adjusts locked textures so they stay where they
	//	had oughta if their brush is rotated, moved or sheared.
...
#pragma message ("this is an ugly hack.  Values should never be == 0.")
	...
	{
		case 0:			//	sides
			geXForm3d_SetZRotation (&xfTexture, Units_DegreesToRadians (t->Rotate));
			geXForm3d_Multiply (&t->XfmFaceAngle, &xfTexture, &xfTexture);	
			geVec3d_Set (&t->TVecs.uVec, -xfTexture.AX, -xfTexture.BX, -xfTexture.CX);
			geVec3d_Set (&t->TVecs.vVec, -xfTexture.AY, -xfTexture.BY, -xfTexture.CY);
