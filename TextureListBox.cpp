		...
		case ADJUST_MODE_FACE :
			{
				...
				for (i = 0; i < Size; ++i)
				{
					...
					if (pFace)
					{
						::TextureFace (pFace, SelId, (LPCSTR)m_currentTextureName);
					}
				}
		...
		case ADJUST_MODE_BRUSH :
			{
				if(m_pDoc->GetSelState() & MULTIBRUSH)
				{
					...
					for (i = 0; i < NumSelBrushes; ++i)
					{
						...
						if (pBrush)
						{
							::TextureBrush (pBrush, SelId, (LPCSTR)m_currentTextureName);
							Brush_UpdateChildFaces (pBrush);
						}
					}
				}
				...
				{
					::TextureBrush (m_pDoc->m_pCurBrush, SelId, (LPCSTR)m_currentTextureName);
