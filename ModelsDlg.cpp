...
void CModelsDlg::OnAddmodel() 
{
	...
	if (rslt == IDOK)
	{
		...
		if (ModelList_Add (pModelInfo->Models, ModelName, pDoc->DocVarsGetPointer()->m_pSelBrushes))
		{
			Model *m;
...
// This sets the model brushes' model id to 0.
{
	...
	{
		ModelList_Remove (pModelInfo->Models, pModelInfo->CurrentModel, pDoc->m_pLevelMgr->GetBrushes (pDoc->GetLevel()));
		PrivateUpdate ();
		pDoc->RebuildTrees ();
	}
}
	...
	)
{
	...
	NumSelBrushes = SelBrushList_GetSize (pDoc->DocVarsGetPointer()->m_pSelBrushes);
	for (iBrush = 0; iBrush < NumSelBrushes; iBrush++)
	{
		Brush *pBrush = NULL;
		int BrushModelId;

		pBrush = SelBrushList_GetBrush (pDoc->DocVarsGetPointer()->m_pSelBrushes, iBrush);
		if (pBrush)
		{
...
// Add selected brushes to this model
{
	...
	if (AddBrushes)
	{
		Model_AddBrushes (pModel, pDoc->DocVarsGetPointer()->m_pSelBrushes);

		// make sure that all model brushes are selected if this is a locked model...
		Brush *pBrush = NULL;
		pBrush = SelBrushList_GetBrush (pDoc->DocVarsGetPointer()->m_pSelBrushes, 0);
...
// remove selected brushes from this model
{
	...
	if (pModel)
	{
		Model_RemoveBrushes (pModel, pDoc->DocVarsGetPointer()->m_pSelBrushes);
		pDoc->RebuildTrees ();
	}
}
...
// This will deselect any other current selection??
{
	...
	if (pModel != NULL)
	{
		...
		pDoc->m_iCurrentTool=ID_TOOLS_BRUSH_MOVEROTATEBRUSH;
...
bool CModelsDlg::UpdateModels( CGEditProDoc *aDoc, ModelInfo_Type *aModelInfo)
{
//	if (pDoc != aDoc)
	{
		pDoc = aDoc;
	...
	)
{
	...
	Model_TransformFromTo (pModel, &XfmWork, &XfmCurrent, pDoc->m_pLevelMgr->GetBrushes (pDoc->GetLevel()));
...
// delete the selected key
{
	...
	if (pModel != NULL)
	{
		...
		if ((CurSel != LB_ERR) && (KeyType == KEY_TYPE_KEYFRAME) && (Time != 0.0f))  // can't delete first key!!
		{
			...
			Model_TransformFromTo (pModel, &XfmFrom, &XfmTo, pDoc->m_pLevelMgr->GetBrushes (pDoc->GetLevel()));
...
// Create ModelOrigin entity and place it at the model's current origin
{
	...
	if (pDoc->CreateEntityFromName ("ModelOrigin", Ent))
	{
		...
		Ent.SetOrigin (org.X, org.Y, org.Z, pDoc->m_pLevelMgr->GetEntityDefs (pDoc->GetLevel()));
		EntityIndex = pDoc->m_pLevelMgr->AddEntity (pDoc->GetLevel(), Ent);
