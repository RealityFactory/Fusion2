...
//		ConPrintf("WARNING:  Allocation failure in Render_AllocViewVars()\n"); // old gedit
		CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("WARNING:  Allocation failure in Render_AllocViewVars()\n"); // new g3dc
...
{
	geVec3d		ystep, xstep, Delt, startDelt, Delt2;
	//, Test={1.0f, 1.0f, 1.0f};
	int			i, cnt, countY, xaxis, yaxis, inidx;
...
//	********************	New for 0.6	*****************************************
	...
	countY	=Units_Round((VectorToSUB(ViewBox.Max, xaxis) - VectorToSUB(ViewBox.Min, xaxis))*gsinv);
	...
	for(i=0;i <= cnt;i++)
	{
		...
		{
			sp	=Render_OrthoWorldToView(v, &Delt);
			MoveToEx(ViewDC, sp.x, sp.y, NULL);
			LineTo(ViewDC, sp.x+1, sp.y);
			...
			else
			{
				if (startDelt.X != 0.0)
					Delt.X = startDelt.X;
				if (startDelt.Z != 0.0 && v->ViewType == VIEWSIDE)
					Delt.Z = startDelt.Z;
			}
		}
		sp = Render_OrthoWorldToView(v, &Delt2);
...
//		ConPrintf("Could not blit to screen!");	//	old gedit
		CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("Could not blit to screen!");	//	new g3dc
		assert(0);
	}
}
