...
{
	Brush		*to = NULL;
	FaceList	*NewFaces = NULL;
	BrushList	*MBList = NULL;
...
{
	int		i;
	geVec3d	vecFixOrg, vecBrushOrg, vecScale;
	...
	geVec3d_Add(&b->BoundingBox.Min, &b->BoundingBox.Max, &vecBrushOrg);
	geVec3d_Scale(&vecBrushOrg, 0.5f, &vecBrushOrg);

	//find the corner of the bounds to keep fixed
	VectorToSUB(vecFixOrg, inidx) = 0.0f;
	if ((sides&3) == 0)	//center x
	{
		dx = -dx;
		VectorToSUB(vecFixOrg, axidx[inidx][0])	= VectorToSUB(vecBrushOrg, axidx[inidx][0]);
	}
	else if((sides&3)==2)	//less x
	{
		VectorToSUB(vecFixOrg, axidx[inidx][0])	= VectorToSUB(b->BoundingBox.Min, axidx[inidx][0]);
	}
	else if((sides&3)==1)	//greater x
	{
		dx = -dx;
		VectorToSUB(vecFixOrg, axidx[inidx][0])	= VectorToSUB(b->BoundingBox.Max, axidx[inidx][0]);
	}

	if((sides&0x0c)==0)	//center y
	{
		VectorToSUB(vecFixOrg, axidx[inidx][1])	= VectorToSUB(vecBrushOrg, axidx[inidx][1]);
	}
	else if((sides&0x0c)==4)	//less y
	{
		dy = -dy;
		if(inidx!=1)
			VectorToSUB(vecFixOrg, axidx[inidx][1])	= VectorToSUB(b->BoundingBox.Min, axidx[inidx][1]);
		else
			VectorToSUB(vecFixOrg, axidx[inidx][1])	= VectorToSUB(b->BoundingBox.Max, axidx[inidx][1]);
	}
	...
	geVec3d_Inverse(&vecFixOrg);
	Brush_Move(b, &vecFixOrg);
	...
	VectorToSUB(vecScale, inidx)			=1.0f;
	VectorToSUB(vecScale, axidx[inidx][0])	=dx;
	VectorToSUB(vecScale, axidx[inidx][1])	=dy;

	for (i=0; i<3; i++)
		VectorToSUB(*fnscale, i) *=VectorToSUB(vecScale, i);

	(*ScaleNum)++;

	Brush_Scale3d(b, &vecScale);
...
void	Brush_SealFaces(Brush **b)
{
	...
	{
		CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("Overconstrained brush clipped away...\n"); // new g3dc
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_GetSelectedFace", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_GetBoundingBox", MB_OK);	//	post 0.55
	...
	if (inList)
	{
		...
		if (outList)
		{
			for(b=inList->First;b;b=b->Next)
			{
				...
				//	need to update the document's "Selected Brush List"
				//	so the new list reflects selected brushes that
				//	correspond to the selected brushes in the old list
				if (pDoc)
				{
					if (pDoc->DocVarsGetPointer()->m_pSelBrushes)
					{
						if (SelBrushList_Find (pDoc->DocVarsGetPointer()->m_pSelBrushes, b))
						{
							SelBrushList_Remove (pDoc->DocVarsGetPointer()->m_pSelBrushes, b);
							SelBrushList_Add (pDoc->DocVarsGetPointer()->m_pSelBrushes, b2);
						}
					}
				}
			}
...
static void	Brush_UpdateChildFacesRecurse(Brush *b, Brush *bp)
{
	...
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_UpdateChildFaceRecurse", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_UpdateChildFaces", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_SetFaceListDirty", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_GetNextBrush", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_GetPrevBrush", MB_OK);	//	post 0.55
...
void	Brush_ShearFixed(Brush *b, float dx, float dy, int sides, int inidx, geVec3d *fnscale, int *ScaleNum)
{
	...
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_ShearFixed", MB_OK);	//	post 0.55
...
void	Brush_SnapShearNearest(Brush *b, geFloat gsize, int sides, int inidx, int snapside)
{
	...
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_SnapShearNearest", MB_OK);	//	post 0.55
...
//				ConPrintf("Axis crossover detected...\n");	//	old gedit
				CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("Axis crossover detected...\n");	//	new g3dc
#endif
...
//				ConPrintf("Please inform Kenneth you got this message!\n");	// old gedit
				CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("Please inform Kenneth you got this message!\n");	// new g3dc
				return;
			}
			continue;
		}
...
//				ConPrintf("WARNING:  Shear snap inaccurate!\n");	//	old gedit
				CGlobals::GetActiveDocument()->m_pMainFrame->ConPrintf("WARNING:  Shear snap inaccurate!\n");	//	new g3dc
			}
			break;
		}
...
Brush	*Brush_CreateHollowFromBrush(const Brush *b)
{
	...
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_CreateHollowFromBrush", MB_OK);	//	post 0.55
...
{
	if (b == NULL)
		MessageBox(NULL, "Brush == NULL", "Brush_EnumFaces", MB_OK);	//	post 0.55
